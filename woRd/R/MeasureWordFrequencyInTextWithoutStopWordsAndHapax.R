#' Measure Word Frequency in Text without Stop words
#'
#' @param text A text
#' @return A data frame
#' @examples
...
MeasureWordFrequencyInTextWithoutStopWordsAndHapax <- function(text){
  words <- ExtractWordsOfText(text)
  words <- RemoveStopWords(words)
  wordFrequency.df <- as.data.frame(table(words))
  wordFrequencyWithoutHapax.df <- subset.data.frame(wordFrequency.df, wordFrequency.df$Freq > 1)
  return(wordFrequencyWithoutHapax.df)
}
